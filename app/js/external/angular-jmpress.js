...
function jmpressRoot( $rootScope, $compile, jmpress, initialStep ) {
	return {
		restrict: "A",
		transclude: true,
		scope: {
      ...
      element.jmpress("idle", function( step, eventData ){
        $rootScope.$broadcast("jmpress.selected." + step.index());
				console.log("jmpress.selected." + step.index());
      });
...
angular.module( "jmpress", [] )
	.directive( "jmpressRoot", [ "$rootScope", "$compile", "jmpress", "initialStep", jmpressRoot ] )
